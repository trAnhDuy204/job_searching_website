...
const ProfileHeader = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await axiosCandidate.get("/profile/me");
        setProfile(res.data);
        setForm(res.data);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axiosCandidate.put("/profile/me", form);
      setProfile(form);
      setIsOpen(false);
    } catch (err) {
  ...
  const handleUploadAvatar = async (e) => {
    ...
    try {
      const res = await axiosCandidate.post("/profile/upload-avatar", formData, {
